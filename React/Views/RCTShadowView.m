 ...
 * aspectRatioBox rectangle is set. This helps calculate
...
static css_dim_t RCTAspectRatioMeasure(void *context, float width)
{
    ...
    if (shadowView.aspectRatioBox.width > 0) {
        computedHeight = width * (shadowView.aspectRatioBox.height / shadowView.aspectRatioBox.width);
    }
    
    css_dim_t result;
    result.dimensions[CSS_WIDTH] = ceilf(width);
    result.dimensions[CSS_HEIGHT] = ceilf(computedHeight);
    return result;
}
